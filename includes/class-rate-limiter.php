 ...
 * Rate Limiter Class
 *
 * Handles API rate limiting to comply with Dropshipzone API throttle limits:
...
/**
 * Rate Limiter for Dropshipzone API
 */
class Rate_Limiter {
    ...
    /**
     * Get recommended delay between requests to stay within limits
     * 
     * This calculates an optimal delay to spread requests evenly
     *
     * @return float Seconds to delay (can be fractional)
     */
    public function get_recommended_delay() {
        // Base: 60 requests per minute = 1 request per second
        // Add buffer: aim for 50 per minute = 1.2 seconds between requests
        $base_delay = 1.2;

        // If approaching limits, increase delay
        $minute_usage = $this->get_minute_count() / self::MAX_PER_MINUTE;
        $hour_usage = $this->get_hour_count() / self::MAX_PER_HOUR;

        if ($minute_usage > 0.8 || $hour_usage > 0.8) {
            $base_delay = 2.0; // Slow down significantly
        } elseif ($minute_usage > 0.5 || $hour_usage > 0.5) {
            $base_delay = 1.5; // Moderate slowdown
        }

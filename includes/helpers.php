...
function dsz_validate_credentials($email, $password) {
    ...
    if (empty($email) || !is_email($email)) {
        return new \WP_Error('invalid_email', __('Please enter a valid email address.', 'dropshipzone-sync'));
    }
    
    if (empty($password)) {
        return new \WP_Error('empty_password', __('Password cannot be empty.', 'dropshipzone-sync'));
    }
...
function dsz_format_datetime($timestamp) {
    if (empty($timestamp)) {
        return __('Never', 'dropshipzone-sync');
    }
...
function dsz_time_ago($timestamp) {
    ...
    return human_time_diff($timestamp, current_time('timestamp')) . ' ' . __('ago', 'dropshipzone-sync');
}
